...
const SocialMediaNuevo = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    try {
      const res = await fetch(`${serverAPIsLocal}/api/redesSociales`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData)
      });
      if (!res.ok) throw new Error("Error al crear la red social");
      navigate("/redesSociales");
    } catch (err) {
      setError("No se pudo crear la red social");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-white shadow-lg rounded-xl p-6 max-w-2xl mx-auto mt-8">
      <h2 className="text-xl font-bold mb-4">Nueva Red Social</h2>
      {loading ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          ...
          <div className="flex justify-end space-x-4 pt-6">
            ...
            >
              {loading ? 'Guardando...' : 'Crear Red Social'}
            </button>
          </div>
        </form>
      )}
...
export default SocialMediaNuevo;
