...
export default function BannerHomeList() {
  ...
  // Ordena por idBannerHome descendente por defecto
  const [sortBy, setSortBy] = useState("idBannerHome");
  ...
  const loadBanners = () => {
    setLoading(true);
    fetch(`${API_BASE_URL}/api/bannerhome`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleSort = (col) => {
    if (sortBy === col) {
      ...
      setSortBy(col);
      setSortDir(col === "idBannerHome" ? "desc" : "asc"); // idBannerHome siempre inicia descendente
    }
    setPage(1);
  };

  // Cambiar estado (activar/desactivar) solo muestra loading en el botón
  const handleStatus = async (id, currentStatus) => {
    try {
      setStatusLoadingId(id);
      const getResponse = await fetch(`${API_BASE_URL}/api/bannerhome/${id}`);
      ...
      const updateResponse = await fetch(`${API_BASE_URL}/api/bannerhome/${id}`, {
        method: "PUT",
      ...
      setBanners(banners.map(b =>
        b.idBannerHome === id ? { ...b, status: nuevoStatus } : b
      ));
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-4 gap-2">
        <div>
          ...
          <Text className="text-gray-500">
            Administración de banners del home
          </Text>
        </div>
        <Button color="blue" className="shadow px-4" onClick={() => navigate("/bannerhome/nuevo")}>
          ...
          <thead className="bg-gray-50">
            <tr>
              <th
                ...
                onClick={() => handleSort("idBannerHome")}
              >
                Id {sortIcon("idBannerHome")}
              </th>
              <th
                className="px-4 py-2 text-left cursor-pointer select-none"
              ...
              paginated.map((item) => (
                ...
                >
                  ...
                  <td className="px-4 py-2 space-x-2">
                    ...
                    <button
                      onClick={() => handleStatus(item.idBannerHome, item.status)}
                      ...
                      disabled={statusLoadingId === item.idBannerHome}
                    >
                      {statusLoadingId === item.idBannerHome ? (
