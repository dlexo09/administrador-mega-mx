...
export default function BannerAvisosList() {
  const [banners, setBanners] = useState([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [page, setPage] = useState(1);
  // Ordena por idBanner descendente por defecto
  const [sortBy, setSortBy] = useState("idBanner");
  ...
  const loadBanners = () => {
    setLoading(true);
    fetch(`${API_BASE_URL}/api/bannersfooter`)
      .then((res) => res.json())
      .then((data) => {
  ...
  // Filtro por nombre del banner
  const filtered = banners.filter((item) =>
    (item.nameBanner || "").toLowerCase().includes(search.toLowerCase())
  );
  ...
  const handleSort = (col) => {
    if (sortBy === col) {
      ...
      setSortBy(col);
      setSortDir(col === "idBanner" ? "desc" : "asc"); // idBanner siempre inicia descendente
    }
    setPage(1);
  };
  ...
  const handleStatus = async (id, currentStatus) => {
    try {
      ...
      const getResponse = await fetch(`${API_BASE_URL}/api/bannersfooter/${id}`);
      if (!getResponse.ok) throw new Error("Error al obtener banner");
      const bannerCompleto = await getResponse.json();
      const nuevoStatus = currentStatus === 1 ? 0 : 1;
      const datosActualizados = {
        ...bannerCompleto,
        status: nuevoStatus
      };
      const updateResponse = await fetch(`${API_BASE_URL}/api/bannersfooter/${id}`, {
        method: "PUT",
      ...
      setBanners(banners.map(b =>
        b.idBanner === id ? { ...b, status: nuevoStatus } : b
      ));
      alert(`Banner ${nuevoStatus === 1 ? "activado" : "desactivado"} correctamente`);
    } catch (e) {
      console.error("Error al actualizar status:", e);
      alert("No se pudo actualizar el status del banner.");
    } finally {
  ...
  return (
    <Card className="bg-white shadow-lg rounded-xl p-6">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-4 gap-2">
        <div>
          <Title>Banners Avisos</Title>
          <Text className="text-gray-500">
            Administración de banners de avisos
          </Text>
        </div>
        <Button color="blue" className="shadow px-4" onClick={() => navigate("/bannerAvisos/nuevo")}>
          ...
          <thead className="bg-gray-50">
            <tr>
              <th
                ...
                onClick={() => handleSort("idBanner")}
              >
                Id {sortIcon("idBanner")}
              </th>
              <th
                className="px-4 py-2 text-left cursor-pointer select-none"
                onClick={() => handleSort("nameBanner")}
              >
                Nombre {sortIcon("nameBanner")}
              </th>
              <th
                className="px-4 py-2 text-left cursor-pointer select-none"
          ...
          <tbody>
            {loading ? (
              ...
              paginated.map((item) => (
                ...
                >
                  <td className="px-4 py-2">{item.idBanner}</td>
                  <td className="px-4 py-2">{item.nameBanner}</td>
                  <td className="px-4 py-2">{item.descripcion || "Sin descripción"}</td>
                  ...
                  <td className="px-4 py-2 space-x-2">
                    ...
                    <Link
                      to={`/bannerAvisos/editar/${item.idBanner}`}
                    ...
                    <button
                      onClick={() => handleStatus(item.idBanner, item.status)}
                      ...
                      disabled={statusLoadingId === item.idBanner}
                    >
                      {statusLoadingId === item.idBanner ? (
